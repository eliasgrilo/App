...
// Type alias for Production component props
...
function KanbanIcon({ active }: IconProps) {
  return (
...
function RecipesIcon({ active }: IconProps) {
  return (
    <svg className={`w-4 h-4 transition-all ${active ? 'scale-110' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={active ? 2.5 : 2}>
...
function CostsIcon({ active }: IconProps) {
  return (
    <svg className={`w-4 h-4 transition-all ${active ? 'scale-110' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={active ? 2.5 : 2}>
...
function FichaIcon({ active }: IconProps) {
  return (
    <svg className={`w-4 h-4 transition-all ${active ? 'scale-110' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={active ? 2.5 : 2}>
...
function ProductionIcon({ active }: IconProps) {
  return (
    <svg className={`w-4 h-4 transition-all ${active ? 'scale-110' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={active ? 2.5 : 2}>
...
function SuppliersIcon({ active }: IconProps) {
  return (
    <svg className={`w-4 h-4 transition-all ${active ? 'scale-110' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={active ? 2.5 : 2}>
...
function ProductsIcon({ active }: IconProps) {
  return (
    <svg className={`w-4 h-4 transition-all ${active ? 'scale-110' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={active ? 2.5 : 2}>
...
export default function App() {
  ...
  // Default view - persistence can be added via Zustand settings store
  ...
  // Get current view label - used in mobile header
  const _currentLabel = NAV_ITEMS.find(item => item.key === view)?.label || 'Menu'
      ...
      <CurrencyProvider>
        <ModalProvider>
          <div className={`min-h-screen bg-gradient-to-br from-zinc-50 via-white to-zinc-100 text-zinc-900 dark:from-black dark:via-zinc-950 dark:to-black dark:text-zinc-100 transition-colors duration-700 ${isLoaded ? 'opacity-100' : 'opacity-0'}`}>
                      ...
                      {NAV_ITEMS.map(({ key, label, icon: Icon }) => {
                        ...
                        return (
                          <button
                            key={key}
                            onClick={() => handleViewChange(key)}
                            className={`relative px-4 py-2.5 rounded-xl transition-colors duration-[250ms] ${isActive
            ...
            {/* Mobile Floating Settings Button - Apple FAB Style */}
            <motion.button
            ...
            <CommandPalette
              ...
              onAction={(action: string, data: unknown) => {
                ...
                // Handle actions like 'new-recipe', 'view-recipe', etc.
              }}
              data={{}}
            />
          </div>
