import React, { useState, useEffect, useMemo, useRef, useCallback, Dispatch, SetStateAction } from 'react'
...
import { useAppStore, useRecipes, useIngredients } from './stores/useAppStore'
import { Ingredient, Recipe, ID, NewRecipe, RecipeIngredient } from './types'

// ═══ LOCAL TYPE DEFINITIONS ═══
type UnitType = 'g' | 'kg' | 'ml' | 'L' | 'un' | 'cx'
...
const calculatePricePerBaseUnit = (inventoryItem: InventoryItemLocal | null): number => {
...
function CreatePizzaModal({ newPizzaName, setNewPizzaName, setIsCreatingPizza, handleCreatePizza }: CreatePizzaModalProps) {
    useScrollLock(true)
...
export default function FichaTecnica() {
    // Zustand Store - using recipes as pizzas (FichaTecnica = pizza recipes)
    const storeRecipes = useRecipes()
    const storeIngredients = useIngredients()
    const { addRecipe, updateRecipe, removeRecipe } = useAppStore()

    // Use store recipes as pizzas (with local wrapper for compatibility)
     
    const pizzas = storeRecipes as unknown as Pizza[]
    const setPizzas = (updater: (prev: Pizza[]) => Pizza[]): void => {
        if (typeof updater === 'function') {
            const newPizzas = updater(pizzas)
            // Sync to store - handled via individual actions
        }
    }
    ...
    const calculateTotals = (ingredients: any[] = []): { totalCost: number; costPerPizza: number } => {
        if (!Array.isArray(ingredients)) return { totalCost: 0, costPerPizza: 0 }

        const totalCost = ingredients.reduce((sum: number, ing: PizzaIngredient) => {
    ...
    // Create new pizza - uses Zustand addRecipe
    ...
    // Delete pizza - uses Zustand removeRecipe
    const handleDeletePizza = (id: ID): void => {
    ...
    // Rename pizza - uses Zustand updateRecipe
    const handleRenamePizza = (id: ID): void => {
        const pizza = pizzas.find(p => p.id === id)
        if (!pizza) return
    ...
    const handleIngredientNameChange = (name: string): void => {
        ...
        if (name && name.trim().length > 0) {
            ...
            }) as InventoryItemLocal | undefined
    ...
    // Add ingredient to selected pizza - uses Zustand updateRecipe
    ...
    // Update ingredient - uses Zustand updateRecipe
    const handleUpdateIngredient = (ingredientId: ID, field: string, value: string | number): void => {
    ...
    // Delete ingredient - uses Zustand updateRecipe
    const handleDeleteIngredient = (ingredientId: ID): void => {
        if (!selectedPizza) return
    ...
    const getItemCost = (ing: PizzaIngredient): number => {
                ...
                <div className="relative z-10 space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-300">
                    ...
                    {!isAddingIngredient && (
                        <button
                            ...
                            className="w-full px-8 py-5 bg-white dark:bg-zinc-950 text-zinc-600 dark:text-zinc-400 rounded-[2rem] font-bold text-xs uppercase tracking-widest hover:bg-zinc-50 dark:hover:bg-zinc-900 border-2 border-dashed border-zinc-200/80 dark:border-zinc-700 transition-all active:scale-[0.99] flex items-center justify-center gap-3 group shadow-sm"
                        >
                    ...
                    {isAddingIngredient && createPortal(
                        <div className="fixed inset-0 z-[10000] flex items-start md:items-center justify-center">
                            ...
                            <motion.div
                                ...
                                className="relative w-full md:max-w-lg bg-white dark:bg-zinc-900 md:bg-white/95 md:dark:bg-zinc-900/95 md:backdrop-blur-2xl rounded-2xl md:rounded-[24px] p-6 md:p-8 shadow-2xl max-h-[85vh] overflow-y-auto custom-scrollbar mx-4 md:mx-0 mt-16 md:mt-0"
                                style={{
                                    marginTop: 'max(calc(env(safe-area-inset-top, 0px) + 60px), 60px)'
                                }}
                            >
                                ...
                                {!matchedInventoryItem && (
                                    <div className="mb-5">
                                        ...
                                        <select
                                            className="w-full px-4 py-4 rounded-2xl bg-zinc-50/50 dark:bg-black/20 border border-zinc-100/80 dark:border-white/5 text-zinc-900 dark:text-white text-lg font-bold focus:outline-none focus:bg-white dark:focus:bg-black/40 focus:ring-1 focus:ring-indigo-500/20 transition-all appearance-none text-center"
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-5">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            step="any"
                                            inputMode="decimal"
                                            className="w-full px-4 py-4 rounded-2xl bg-zinc-50/50 dark:bg-black/20 border border-zinc-100/80 dark:border-white/5 text-zinc-900 dark:text-white text-right text-lg font-bold focus:outline-none focus:bg-white dark:focus:bg-black/40 focus:ring-1 focus:ring-indigo-500/20 transition-all"
                                            placeholder="0"
                                    ...
                                    {matchedInventoryItem && (
                                        <div>
                                            <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-2">Unidade</label>
                                            <div className="w-full px-4 py-4 rounded-2xl bg-zinc-100 dark:bg-zinc-800 border border-zinc-100/80 dark:border-zinc-700 text-zinc-400 dark:text-zinc-500 text-lg font-semibold text-center">
                                                {newIngredient.unit}
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {editingId === ing.id ? (
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div>
                                                ...
                                                <input
                                                    type="text"
                                                    className="w-full px-4 py-3 rounded-xl bg-zinc-50/50 dark:bg-black/20 border border-zinc-100/80 dark:border-white/5 text-zinc-700 dark:text-zinc-200 font-medium focus:outline-none focus:bg-white dark:focus:bg-black/40 focus:ring-1 focus:ring-indigo-500/30 transition-all text-sm placeholder:text-zinc-300"
                                                    value={ing.name}
                                                    onChange={(e) => handleUpdateIngredient(ing.id, 'name', e.target.value)}
                                                    onBlur={() => {
                                                        if (!ing.name.trim() && (!ing.quantity || ing.quantity === 0)) handleDeleteIngredient(ing.id)
                                            ...
                                            <div className="grid grid-cols-2 gap-3">
                                                <div>
                                                    ...
                                                    <input
                                                        ...
                                                        step="any"
                                                        inputMode="decimal"
                                                        className="w-full px-4 py-3 rounded-xl bg-zinc-50/50 dark:bg-black/20 border border-zinc-100/80 dark:border-white/5 text-zinc-900 dark:text-white font-semibold text-lg text-right focus:outline-none focus:bg-white dark:focus:bg-black/40 focus:ring-1 focus:ring-indigo-500/30 transition-all"
                                                        value={ing.quantity}
                                                        onChange={(e) => handleUpdateIngredient(ing.id, 'quantity', e.target.value)}
                                                        onBlur={() => {
                                                            if (!ing.name.trim() && (!ing.quantity || ing.quantity === 0)) handleDeleteIngredient(ing.id)
                                                ...
                                                <div>
                                                    ...
                                                    <div className="relative">
                                                        <select
                                                            className="w-full px-4 py-3 rounded-xl bg-zinc-50/50 dark:bg-black/20 border border-zinc-100/80 dark:border-white/5 text-zinc-600 dark:text-zinc-300 font-semibold text-center appearance-none focus:outline-none focus:bg-white dark:focus:bg-black/40 focus:ring-1 focus:ring-indigo-500/30 transition-all text-sm"
                                                            value={ing.unit}
                                                            onChange={(e) => handleUpdateIngredient(ing.id, 'unit', e.target.value)}
                                                        >
                                                            <option value="g">g</option>
                                        ...
                                        <div onClick={() => setEditingId(ing.id)} className="group cursor-pointer">
                                            <div className="flex items-start justify-between mb-3">
                                                <div className="flex-1">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <span className="inline-flex items-center px-2 py-0.5 rounded-md bg-zinc-50 dark:bg-white/5 border border-zinc-100/80 dark:border-white/5 text-[10px] font-bold text-zinc-500 dark:text-zinc-400 tabular-nums uppercase tracking-wide">
                                            ...
                                            <div className="flex items-center justify-between pt-3 border-t border-dashed border-zinc-100/80 dark:border-white/5">
