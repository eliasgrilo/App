...
// Image compression placeholder - can be enhanced later
const compressImage = async (file: File): Promise<File> => file
...
const ImageLightbox = ({ src, onClose }: ImageLightboxProps): React.ReactElement => {
    useScrollLock(true)
    return createPortal(
        <motion.div
...
const ImageCropperModal = ({ imageSrc, onCancel, onCropComplete }: ImageCropperModalProps): React.ReactElement => {
    ...
    const onCropCompleteInternal = (croppedArea: any, croppedAreaPixels: any): void => {
...
const RecipeCategoryModal = ({ categories, onClose, onUpdate, onRenameCategory }: RecipeCategoryModalProps): React.ReactElement => {
...
const IngredientItem = React.memo(({ item, onUpdate, onDelete, onNext, isEditing }: IngredientItemProps) => {
...
const InstructionItem = React.memo(({ item, index, onUpdate, onDelete, onNext, isEditing }: InstructionItemProps) => {
    const dragControls = useDragControls()
    const [checked, setChecked] = useState(false)
        ...
        >
            <div className={`flex gap-3 py-3 px-1 rounded-xl transition-all duration-[250ms] ease-[cubic-bezier(0.4,0,0.2,1)] ${isEditing ? 'hover:bg-zinc-50/50 dark:hover:bg-zinc-900/30' : checked ? 'opacity-50' : 'hover:bg-zinc-50 dark:hover:bg-zinc-900/10'}`}>
                ...
                {isEditing && (
                    <button
                        onClick={onDelete}
                        className="p-3 text-zinc-300 dark:text-zinc-700 hover:text-rose-500 dark:hover:text-rose-400 transition-colors duration-[250ms] opacity-100 md:opacity-0 md:group-hover:opacity-100 mt-0.5 touch-manipulation"
                    >
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                        </svg>
...
const getCategoryColor = (categories: any[], categoryName: string) => {
