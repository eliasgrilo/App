...
interface InventoryItem { id: number; name: string; unit: string; expirationDate?: string; packageQuantity?: number; packageCount?: number }
...
const getExpirationStatus = (item: InventoryItem): ExpirationStatus => {
    if (!item.expirationDate) return 'noDate'
...
const StatusTile = ({ tile, isSelected, onClick }: { tile: { id: string; label: string; count: number; color: string }; isSelected: boolean; onClick: () => void }) => {
    ...
    const hasPulse = (tile.id === 'expired' || tile.id === 'critical') && tile.count > 0
...
const ExpirationItemRow = ({ item, status, total, unit, onClick }: { item: InventoryItem; status: ExpirationStatus; total: number; unit: string; onClick: () => void }) => {
    ...
    return (
        <motion.div onClick={onClick} whileHover={{ scale: 1.003 }} whileTap={{ scale: 0.997 }}
            ...
            style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.85) 0%, rgba(255,255,255,0.65) 100%)', boxShadow: '0 0 0 1px rgba(0,0,0,0.02), 0 2px 8px -2px rgba(0,0,0,0.03)' }}>
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {item.expirationDate && (
                        <div className="mt-1.5 flex items-center gap-2">
                            <svg className="w-3 h-3 text-zinc-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                            <span className="text-[10px] text-zinc-400">{new Date(item.expirationDate).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                        </div>
                    )}
                </div>
                <motion.svg className="w-4 h-4 text-zinc-200 flex-shrink-0 transition-all group-hover:text-zinc-400 group-hover:translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></motion.svg>
            </div>
        </motion.div>
    )
}
...
export function ExpirationSection({ items, getTotalQuantity, onConfigureItem }: ExpirationSectionProps): React.ReactElement {
    // Smart initial filter: Expired > Critical > Warning
    ...
    const tiles = [
        ...
        { id: 'critical', label: '≤3 dias', count: items.filter(i => getExpirationStatus(i) === 'critical').length, color: 'red', filter: 'critical' },
        { id: 'warning', label: '≤7 dias', count: items.filter(i => getExpirationStatus(i) === 'warning').length, color: 'orange', filter: 'warning' },
    ...
    const filteredItems = items
        ...
        .sort((a, b) => { if (!a.expirationDate) return 1; if (!b.expirationDate) return -1; return new Date(a.expirationDate).getTime() - new Date(b.expirationDate).getTime() })
        .slice(0, 10)
    ...
    return (
        <section className="relative z-10 mb-10">
            ...
            <motion.div initial={{ opacity: 0, y: 20, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }} className="relative">
                ...
                <div className="relative overflow-hidden rounded-[2.5rem]" style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.95) 0%, rgba(255,255,255,0.85) 100%)', boxShadow: '0 0 0 1px rgba(255,255,255,0.5) inset, 0 25px 50px -12px rgba(0,0,0,0.08), 0 0 80px -20px rgba(255,149,0,0.15)', backdropFilter: 'blur(40px) saturate(180%)' }}>
                    <div className="absolute inset-0 rounded-[2.5rem] pointer-events-none" style={{ background: 'linear-gradient(180deg, rgba(255,255,255,0.6) 0%, transparent 40%)' }} />
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="px-7 pt-7 pb-4">
                            <div className="flex items-center justify-between gap-4">
                                <div className="flex-1 min-w-0">
                                    <motion.h2 initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-xl font-semibold text-zinc-900 tracking-tight leading-none">Validade</motion.h2>
                                ...
                                <div className="flex items-center gap-2">
                                        ...
                                        {isFocused ? (
                                            <motion.div key="input" initial={{ width: 0, opacity: 0 }} animate={{ width: 180, opacity: 1 }} exit={{ width: 0, opacity: 0 }} transition={{ duration: 0.2, ease: [0.4, 0, 0.2, 1] }} className="relative">
                                                <input autoFocus type="text" placeholder="Buscar..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onBlur={() => { if (!searchQuery) setIsFocused(false) }}
                                                    className="w-full pl-3 pr-8 py-1.5 rounded-lg text-[12px] text-zinc-800 font-medium bg-white/90 placeholder:text-zinc-300 focus:outline-none border border-zinc-200/60 focus:border-amber-200/60" />
                                            ...
                                            <motion.button key="icon" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} onClick={() => setIsFocused(true)}
                                                className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-zinc-100/80 transition-colors">
                                    ...
                                    <motion.div whileHover={{ scale: 1.03 }} className="px-3 py-1.5 rounded-lg" style={{ background: 'rgba(255,149,0,0.06)', boxShadow: '0 0 0 1px rgba(255,149,0,0.08) inset' }}>
                                        <span className="text-[11px] font-semibold text-amber-600 tabular-nums">{items.filter(i => i.expirationDate).length}</span>
