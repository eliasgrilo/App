    ...
    <div>
        ...
        <div className="flex flex-wrap gap-2">
            {Object.entries(MOVEMENT_TYPES).map(([k, v]) => (
                <button key={k} onClick={() => setForm(f => ({ ...f, type: k as 'entrada' | 'saida', reasonLabel: REASON_BY_TYPE[k as 'entrada' | 'saida'][0] || '' }))} className={`px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wide transition-all ${type === k ? 'bg-zinc-900 dark:bg-white text-white dark:text-zinc-900' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500'}`}>{v.label}</button>
            ))}
        </div>
    </div>
)
...
export const QuantityFields: React.FC<QuantityFieldsProps> = ({ form, setForm }) => (
    <div className="grid grid-cols-2 gap-3">
        <div>
            ...
            <input type="number" value={form.qty} onChange={e => setForm(f => ({ ...f, qty: e.target.value }))} placeholder="0" className="w-full h-14 px-5 rounded-2xl bg-zinc-100 dark:bg-zinc-800 text-[20px] font-semibold text-zinc-900 dark:text-white placeholder:text-zinc-300 outline-none" />
        </div>
        <div>
...
export const ReasonFields: React.FC<ReasonFieldsProps> = ({ form, setForm }) => (
    <div>
        ...
        <input type="text" value={form.reasonNote} onChange={e => setForm(f => ({ ...f, reasonNote: e.target.value }))} placeholder="Digitar o motivo:" className="w-full h-12 px-4 rounded-xl bg-zinc-100 dark:bg-zinc-800 text-sm text-zinc-900 dark:text-white placeholder:text-zinc-400 outline-none" />
    </div>
)
