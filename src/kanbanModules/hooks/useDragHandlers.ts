...
export function useDragHandlers({ board, dragState, setDragState, ghostX, ghostY, pendingDragRef, rafRef, lastTargetRef, setEditingCard, setBoard, haptic }: UseDragHandlersProps) {
    const findDropTarget = useCallback((clientX: number, clientY: number, activeCardId: string) => {
        ...
        const cards = Array.from(container.querySelectorAll('[data-card-id]')).filter((c: any) => c.getAttribute('data-card-id') !== activeCardId)
    ...
    const handleCardPointerDown = useCallback((e: any, card: any, colId: string) => {
        ...
        pendingDragRef.current = { card, colId, element, rect, startX: e.clientX, startY: e.clientY, offsetX: e.clientX - rect.left, offsetY: e.clientY - rect.top, cardIndex: board.columns.find(c => c.id === colId)?.cards.findIndex((c: any) => c.id === card.id) ?? 0 }
    ...
    useEffect(() => {
        ...
        const handleWindowPointerUp = (e: any) => {
            ...
            if (dragState.isDragging && dragState.active) {
                ...
                if (needsUpdate) {
                    setBoard(prev => { const newCols = prev.columns.map(col => ({ ...col, cards: [...col.cards] })); const sourceCol = newCols.find(c => c.id === active.sourceColId); const destCol = newCols.find(c => c.id === targetColId); if (!sourceCol || !destCol) return prev; const cardIdx = sourceCol.cards.findIndex((c: any) => c.id === active.id); if (cardIdx === -1) return prev; const [movedCard] = sourceCol.cards.splice(cardIdx, 1); destCol.cards.splice(targetIndex || 0, 0, movedCard!); return { ...prev, columns: newCols } })
                    haptic('medium')
                }
