...
export async function createQuotation({
    ...
    const quotation = {
        ...
        status: QUOTATION_STATUS.PENDING, // Direct injection to Pending
        isAutoGenerated: true, // Flag for auto-generated quotations
    ...
    // CRITICAL: Sync to Firestore so IA page can see it immediately
    await syncToFirestore(quotation);
...
export async function processSupplierResponse(quotationId, emailBody, userId, userName) {
    ...
    // BUG FIX: Validate quotation is in valid state for processing
    ...
    if (!validStates.includes(quotation.status)) {
        ...
        return { success: false, error: 'Quotation already processed', quotation };
    }
